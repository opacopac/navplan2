...
function autoLoginUser()
{
    ...
    // check email/token
    $email = checkEscapeEmail($conn, trim($input["email"]));
    $oldToken = checkEscapeAlphaNumeric($conn, trim($input["token"]), 10, 100);

    // find email + token
    $query = "SELECT id FROM users WHERE email='" . $email . "' AND token='" . $oldToken . "'";
    $result = DbService::execSingleResultQuery($conn, $query);
    ...
    {
        $token = "";
        $message = "error: invalid email or token";
        $resultcode = -2;
    }

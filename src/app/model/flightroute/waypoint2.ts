...
export class Waypoint2 {
    ...
    get previousPosition$(): Observable<Position2d> {
        return this.previousWaypoint$
            .switchMap(prevWp => prevWp ? prevWp.position$ : RxService.getEternal<Position2d>());
    }
    ...
    get isDestinationAirport$(): Observable<boolean> {
        return Observable.combineLatest(
            this.type$,
            this.nextWaypoint$,
            this.nextWaypoint$.switchMap(wp => wp ? wp.isAlternate$ : RxService.getEternal<boolean>(false)),
    ...
    get speed$(): Observable<Speed> {
        return this.speedObservable$.switchMap(speedObs => speedObs ? speedObs : RxService.getEternal<Speed>());
    }
    ...
    get nextMt$(): Observable<Angle> {
        return this.nextWaypoint$
            .switchMap(nextWp => nextWp ? nextWp.mt$ : RxService.getEternal<Angle>());
    }
    ...
    get vacTime$(): Observable<Time> {
        return Observable.combineLatest(
            ...
            this.previousWaypoint$,
            this.nextWaypoint$,
            this.previousWaypoint$.switchMap(prevWp => prevWp ? prevWp.isOriginAirport$ : RxService.getEternal<boolean>(false)),
            this.nextWaypoint$.switchMap(nextWp => nextWp ? nextWp.isAlternate$ : RxService.getEternal<boolean>(false))
        )
